...
|WS Human Task transition|CMMN Task/Stage state           |
|------------------------|--------------------------------|
|Activate				 |Enable                          |
|Suspend				 |Suspend                         |
|Resume					 |Resume                          |
...
For Jackrabbit's OCM, we followed a similar approach, and the OcmCasePersistence also requires a JCR EventListener to queue persistence events. Interestingly, the default behaviour for JCR is that these events should be
